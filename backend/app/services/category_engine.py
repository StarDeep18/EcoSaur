...
from app.models.schemas import ProductCategoryInfo, ComparisonCard
...
def classify_product(product_name: str, ingredients: List[str]) -> ProductCategoryInfo:
    ...
    from app.models.database_models import ProductCategory
...
def rank_alternatives(scanned: ProductCategoryInfo) -> List[Dict[str, Any]]:
    """
    Ranks healthy alternatives from our SQLite database using contextual similarity scoring.
    Ensures strict beverage-to-beverage and snack-to-snack containment.
    """
    from app.db.database import SessionLocal
    from app.models.database_models import Alternative, ProductCategory
    ...
    try:
        ...
        for alt in alts:
            ...
            # 1. Strict Category Compliance (Snacks do not suggest drinks, and vice versa)
            ...
            if scanned_is_beverage != alt_is_beverage:
                continue # Disqualify completely

            # 2. Subcategory Match (+50 points)
            if alt_cat.subcategory_name == scanned.subcategory:
                score += 50

            # 3. Flavor Profile Match (+20 points)
            if alt_cat.flavor_profile == scanned.flavor_profile:
                score += 20

            # 4. Craving Type match (e.g. Fizzy, Crunchy, Soft) (+15 points)
            ...
            # 5. Processing Level Reduction (+5 per level reduction)
            reduction = max(0, scanned.processing_level - alt_cat.processing_level)
            score += reduction * 5

            # 6. Convenience Match (+10 points for matches above level 4)
...
def get_comparison_cards(scanned: ProductCategoryInfo, scanned_name: str) -> List[ComparisonCard]:
    """
    Determines 2 commercial healthier products inside the EXACT SAME subcategory, 
    plus a healthy homemade option for side-by-side smart comparison.
    """
