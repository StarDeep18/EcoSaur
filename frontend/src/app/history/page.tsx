    ...
    <div className="container animate-fade-in">
      ...
      {insights && insights.has_scans && (
        <div className="card animate-fade-in" style={{ 
          ...
          <h2 className="title" style={{ fontSize: '1.4rem', color: '#10b981', display: 'flex', alignItems: 'center', gap: '0.4rem', WebkitTextFillColor: 'initial', background: 'none', marginBottom: '0.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem', marginTop: '1rem', marginBottom: '1rem' }}>
            <div style={{ background: 'rgba(0, 0, 0, 0.25)', padding: '0.85rem', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.03)' }}>
              ...
              <span style={{ fontSize: '1.1rem', fontWeight: 700, color: 'white', marginTop: '0.15rem', display: 'block' }}>{insights.common_category}</span>
            </div>
            <div style={{ background: 'rgba(0, 0, 0, 0.25)', padding: '0.85rem', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.03)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            {insights.additive_trends && insights.additive_trends.map((t: string, idx: number) => (
              <p key={idx} style={{ fontSize: '0.85rem', color: '#cbd5e1', lineHeight: '1.4' }}>
          ...
          {insights.swap_suggestions && insights.swap_suggestions.length > 0 && (
            <div style={{ 
              ...
              <span style={{ fontSize: '0.8rem', color: '#34d399', fontWeight: 700, textTransform: 'uppercase', display: 'block', marginBottom: '0.5rem' }}>
                ⭐ Personalized Healthier Swap
              </span>
              {insights.swap_suggestions.map((suggestion: any, idx: number) => (
                <div key={idx} style={{ background: 'rgba(0,0,0,0.15)', padding: '0.75rem', borderRadius: '12px', fontSize: '0.85rem', lineHeight: '1.4' }}>
                  Replace <strong style={{ color: '#fca5a5' }}>{suggestion.original}</strong> with <strong style={{ color: '#6ee7b7' }}>{suggestion.swap}</strong>.
      ...
      {isLoading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '3.5rem 1.5rem' }}>
          ...
          <h3 style={{ fontSize: '1.25rem', marginBottom: '0.5rem', color: 'white' }}>No past scans found</h3>
        ...
        history.map((item, idx) => {
          ...
          return (
            <div key={idx} className="card" style={{ 
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{ margin: 0, fontSize: '1.1rem', color: 'white' }}>Score: {item.score}/100</h3>
                {item.name && (
